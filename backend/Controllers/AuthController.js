...
const signup = async (req, res) => {
  try {
    ...
    const customer = await UserModel.findOne({
      $or: [{ email }, { mobileNumber }, { NationalIDNumber }],
    });
    ...
    if (customer) {
      ...
      } else if (customer.NationalIDNumber === NationalIDNumber) {
        message = "National ID number is already registered. Please login.";
      }
    ...
    const newUser = new UserModel({
      ...
      NationalIDNumber,
      NationalIDPhoto: {
        data: req.files.NationalIDPhoto[0].buffer,
        contentType: req.files.NationalIDPhoto[0].mimetype,
      },
    });

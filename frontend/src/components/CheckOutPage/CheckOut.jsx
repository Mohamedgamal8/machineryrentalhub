...
const CheckOut = () => {
  ...
  useEffect(() => {
    const fetchVehicles = async () => {
      try {
        const response = await fetch(
          `https://machineryrentalhub-backend-production.up.railway.app/api/vehicles/listings/${id}` // Use the ID in the fetch request
        );
        if (!response.ok) {
    ...
    const fetchSimilarVehicles = async () => {
      try {
        const response = await fetch(
          `https://machineryrentalhub-backend-production.up.railway.app/api/vehicles/listings/${id}` // Adjust the endpoint accordingly
        );
        if (!response.ok) {
          ...
          <div className="p-4 flex-1 text-left">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm sm:text-base px-3">
              ...
              <div>
                ...
                <p>
                  {vehicle.price1 ? ` ${vehicle.price1}` : "Price not available"}
                </p>
              </div>
            </div>
          </div>

...
export default function ConstructionRental() {
  ...
  const [budget, setBudget] = useState([0, 100]); // Default budget range for 'All' is full range
    ...
    const fetchVehicles = async () => {
      try {
        const response = await fetch(
          "https://machineryrentalhub-backend-production.up.railway.app/api/vehicles/listings"
        );
        if (!response.ok) {
  ...
  useEffect(() => {
    if (selectedType === "All") {
      setBudget([0, 100]); // Full budget range when "All" is selected
    } else {
      setBudget([0, 100]);
    }
  }, [selectedType]);
                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    ...
                    <div className="flex items-center font-semibold text-lg text-yellow-700">
                      <FaTags className="mr-2" /> <span>Hour {vehicle.priceHour}</span>
                    </div>
                    <div className="flex items-center font-semibold text-lg text-yellow-700">
                      <FaTags className="mr-2" /> <span> Day{vehicle.priceDay}</span>
                    </div>
                  </div>
                </CardContent>
